<?php
/**
 * Add countries as a field type
 */
class BP_XProfile_Country_Field {
	/**
	...
	function __construct() {
		...
		// include class
		require_once dirname( __FILE__ ) . '/bp_countries_class.php';
	...
	function get_field_value( $value = '', $type = '', $id = '' ) {
		...
		if ( $type == 'country' ) {
			...
			$value = apply_filters( 'bp_xprofile_field_type_country_content', stripslashes( $value ) );
			// return filtered value
			return apply_filters( 'bp_xprofile_field_type_country_value', $value );
		}
		// fallback
		return $value;
	}
 ...
 * @return void
 */
function bp_xprofile_country_field() {
	// make global in scope
	global $bp_xprofile_country_field;
	// init plugin
	$bp_xprofile_country_field = new BP_XProfile_Country_Field();
}

// add action for plugin loaded
add_action( 'bp_init', 'bp_xprofile_country_field' );